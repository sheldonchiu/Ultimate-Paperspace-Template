...
if [[ -n $1 ]]; then
    case $1 in
        "controller")
            nohup python3 -m fastchat.serve.controller --host 127.0.0.1 > /tmp/log/fastchat_controller.log 2>&1 &
        ...
        "worker")
            ...
            do
                if [ -n "$model" ]; then
                    (( port++ ))
                    nohup python3 -m fastchat.serve.model_worker --host 127.0.0.1 --port $port --model-path $model --load-8bit ${model_args[$model_args_id]} > /tmp/log/fastchat_worker_$port.log 2>&1 &
            ...
            ;;
        "server")
            nohup python3 -m fastchat.serve.gradio_web_server --model-list-mode once --port $FASTCHAT_PORT > /tmp/log/fastchat_server.log 2>&1 &
